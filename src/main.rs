...
async fn main(spawner: Spawner) {
    ...
    // ── OLED first — so we can see status at every step ──────────────────────
    ...
    oled::print(&mut display, style, &[hdr.as_str(), "WiFi OK", wifi.ip.as_str(), ""]);
    ...
    info!("all init done");
    oled::print(&mut display, style, &[hdr.as_str(), "Ready", wifi.ip.as_str(), ""]);
